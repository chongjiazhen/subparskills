...
2. Run checks fresh over every claim whose inputs, producers, or pinned targets moved. Widen to the full suite when that scope is uncertain or dependencies cross it. Read output and inspect changed state, not only exit status.
...
- Missing evidence blocks the specific claim it would prove, not unrelated work that carries its own evidence.
