...
Finding 2026-09-02: the bundled Claude Code `code-review` skill carries the alias `review`; probed which body a typed `/review` reaches (Claude Code 2.1.258, isolated `CLAUDE_CONFIG_DIR`, haiku, a user-scope `review` skill whose body is a marker string, a repo with one modified file). `/review` returned the marker, `/code-review` ran the bundled multi-angle review, and a plain "use the review skill" returned the marker. The catalog skill wins the typed alias; the bundled reviewer remains on `/code-review` only. Windows note: from Git Bash pass `MSYS_NO_PATHCONV=1` and a `cygpath -w` config dir, or the slash prompt reaches the model as `C:/Program Files/Git/review`.
