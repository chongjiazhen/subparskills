...
- Receiving review: verify each finding against the codebase before implementing it; if any item is unclear, clarify all unclear items before implementing any. Push back with technical reasoning where a finding is wrong - do not comply performatively. A finding that reverses a decision the user made explicitly earlier: stop and confirm with the user before implementing it, however correct it reads.
