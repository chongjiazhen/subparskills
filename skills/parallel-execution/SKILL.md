...
5. When workers commit to one shared checkout: commit by pathspec (`git commit -m "msg" -- <your paths>`) after `git diff --cached --stat` shows only your files. A bare `git commit` commits the whole index and sweeps whatever a sibling has staged into your commit.
