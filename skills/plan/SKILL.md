...
8. Hand off to `implement`, which runs the tasks in order, each in a fresh subagent; independent slices that must run concurrently or across a process boundary go to `parallel-execution` with the persisted file.
