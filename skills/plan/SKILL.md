...
3. Write the plan in the shape of [TEMPLATE.md](TEMPLATE.md): the spec path in the header, a Global Constraints block copied verbatim from the spec, and each task ending in a commit step. Name exact commands and expected result. Remove placeholders and ambiguous steps.
...
6. Hand off: solo in-context execution to `implement`; across a process boundary to `parallel-execution` with the persisted file.
